...
function MapPage({ session }) {
  ...
  useEffect(() => {
    ...
    geoPhotos.forEach((photo) => {
      ...
      popupContent.innerHTML = `
        ...
        <div class="map-popup-info">
          <div class="map-popup-title">
            ...
            <a class="streetview-link" href="#" title="Googleストリートビューで開く">
      ...
      // ストリートビューリンクをタップ時に最新座標で遷移
      const svLink = popupContent.querySelector('.streetview-link')
      if (svLink) {
        svLink.addEventListener('click', (e) => {
          e.preventDefault()
          window.open(
            `https://www.google.com/maps/@?api=1&map_action=pano&viewpoint=${photo.latitude},${photo.longitude}`,
            '_blank'
          )
        })
      }
